...
SEUIL_MORTALITE_ALERTE = (
    2.0  # Taux d'alerte : % de décès par rapport aux hospitalisations
)
SEUIL_RUPTURE_STOCK_JOURS = (
    30  # Nombre de jours minimum de stock de sécurité à maintenir
)
...
total_medecins = (
    h1_nb_medecins + h2_nb_medecins + h3_nb_medecins + h4_nb_medecins + h5_nb_medecins
)
total_population = (
    h1_population_zone
    + h2_population_zone
...
valeur_stock_fcfa = (
    valeur_stock_usd * TAUX_USD_FCFA
)  # Attention à la petite faute de frappe dans votre variable 'TAUX_USD_FCFA'
...
if m1_stock <= m1_seuil_rupture:
    m1_statut, m1_couleur, m1_action = (
        "RUPTURE CRITIQUE",
        "[ROUGE]",
        "Alerte immédiate PNA — commande express sous 24h",
    )
elif m1_stock <= m1_seuil_rupture * 1.5:
    m1_statut, m1_couleur, m1_action = (
        "ALERTE STOCK",
        "[ORANGE]",
        "Commande urgente à déclencher sous 72h",
    )
elif m1_stock <= m1_seuil_rupture * 2.0:
    m1_statut, m1_couleur, m1_action = (
        "STOCK LIMITE",
        "[JAUNE]",
        "Surveillance renforcée — planifier une commande",
    )
else:
    m1_statut, m1_couleur, m1_action = (
        "STOCK NORMAL",
        "[VERT]",
        "Situation normale — suivi standard",
    )

# Médicament 2

if m2_stock <= m2_seuil_rupture:
    m2_statut, m2_couleur, m2_action = (
        "RUPTURE CRITIQUE",
        "[ROUGE]",
        "Alerte immédiate PNA — commande express sous 24h",
    )
...
if m3_stock <= m3_seuil_rupture:
    m3_statut, m3_couleur, m3_action = (
        "RUPTURE CRITIQUE",
        "[ROUGE]",
        "Alerte immédiate PNA — commande express sous 24h",
    )
...
if m4_stock <= m4_seuil_rupture:
    m4_statut, m4_couleur, m4_action = (
        "RUPTURE CRITIQUE",
        "[ROUGE]",
        "Alerte immédiate PNA — commande express sous 24h",
    )
...
if m5_stock <= m5_seuil_rupture:
    m5_statut, m5_couleur, m5_action = (
        "RUPTURE CRITIQUE",
        "[ROUGE]",
        "Alerte immédiate PNA — commande express sous 24h",
    )
...
print("=" * 163)
print(
    f"{'':<44}\t{'TABLEAU DE BORD DU STOCK DE LA PHARMACIE NATIONALE D’APPROVISIONNEMENT (PNA)'}"
)
print("=" * 163)
# Les en-têtes du tableau de suivi des stocks de médicaments

print(
    f"{'Nom du médicament':<35}\t| {'Stock actuel':<10}\t| {'Seuil d’alerte':<0}\t| {'Statut du stock':<0}\t| {'Couleur':<5}\t| {'Action requise':<0}"
)
print("-" * 163)
...
print(
    f"{m1_nom:<35}\t| {m1_stock:<5}unites\t| {m1_seuil_rupture:<12}unites\t| {m1_statut:<15}\t| {m1_couleur:<5}\t| {m1_action:<15}"
)
print("-" * 163)
...
print(
    f"{m2_nom:<35}\t| {m2_stock:<5}unites\t| {m2_seuil_rupture:<12}unites\t| {m2_statut:<15}\t| {m2_couleur:<5}\t| {m2_action:<15}"
)
print("-" * 163)

print(
    f"{m3_nom:<35}\t| {m3_stock:<5}unites\t| {m3_seuil_rupture:<12}unites\t| {m3_statut:<15}\t| {m3_couleur:<5}\t| {m3_action:<15}"
)
print("-" * 163)

print(
    f"{m4_nom:<35}\t| {m4_stock:<5}unites\t| {m4_seuil_rupture:<12}unites\t| {m4_statut:<15}\t| {m4_couleur:<5}\t| {m4_action:<15}"
)
print("-" * 163)

print(
    f"{m5_nom:<35}\t| {m5_stock:<5}unites\t| {m5_seuil_rupture:<12}unites\t| {m5_statut:<15}\t| {m5_couleur:<5}\t| {m5_action:<15}"
)
print("=" * 163)
...
print(
    f"  Médicaments en rupture critique : {nb_ruptures_critiques} ({nom_med_critique})"
)
print(
    f"  Médicaments en alerte stock     : {nb_alertes_stock} ({nom_med_alerte_stock})"
)
print(
    f"  Médicaments en stock limité     : {nb_stock_limites} ({nom_med_stock_limite})"
)
print(
    f"  Médicaments en stock normal     : {nb_stock_normaux} ({nom_med_stock_normaux})"
)
...
if nb_ruptures_critiques > 0:
    print(
        f"  !! ALERTE PRIORITAIRE : {nb_ruptures_critiques} médicament(s) en RUPTURE CRITIQUE !!"
    )
print("=" * 65)
...
print(
    f"{'Nom de l’établissement':<30}\t| {'Total lits':<5}\t| {'Lits occupés':<5}\t| {'Taux d’occupation':<0}\t| {'Statut (Normes OMS)':<5}\t| {'Action requise':<0}"
)
print("-" * 142)
print(
    f"{h1_nom:<30}\t| {h1_nb_lits:<10}\t| {h1_nb_lits_occupes:<10}\t| {taux_occupation_h1:<14}%\t| {status_occ_h1:<5}\t| {action_1:<0}"
)
print("-" * 142)

print(
    f"{h2_nom:<30}\t| {h2_nb_lits:<10}\t| {h2_nb_lits_occupes:<10}\t| {taux_occupation_h2:<14}%\t| {status_occ_h2:<5}\t| {action_2:<0}"
)
print("-" * 142)

print(
    f"{h3_nom:<30}\t| {h3_nb_lits:<10}\t| {h3_nb_lits_occupes:<10}\t| {taux_occupation_h3:<14}%\t| {status_occ_h3:<5}\t| {action_3:<0}"
)
print("-" * 142)

print(
    f"{h4_nom:<30}\t| {h4_nb_lits:<10}\t| {h4_nb_lits_occupes:<10}\t| {taux_occupation_h4:<14}%\t| {status_occ_h4:<5}\t| {action_4:<0}"
)
print("-" * 142)

print(
    f"{h5_nom:<30}\t| {h5_nb_lits:<10}\t| {h5_nb_lits_occupes:<10}\t| {taux_occupation_h5:<14}%\t| {status_occ_h5:<5}\t| {action_5:<0}"
)
print("=" * 142)
# ======================== Compteurs triés selon le statut OMS attendu ========================================================

nb_hopitaux_saturation = 0
if status_occ_h1 == "[ALERTE CRITIQUE]":
    nb_hopitaux_saturation += 1
if status_occ_h2 == "[ALERTE CRITIQUE]":
    nb_hopitaux_saturation += 1
if status_occ_h3 == "[ALERTE CRITIQUE]":
    nb_hopitaux_saturation += 1
if status_occ_h4 == "[ALERTE CRITIQUE]":
    nb_hopitaux_saturation += 1
if status_occ_h5 == "[ALERTE CRITIQUE]":
    nb_hopitaux_saturation += 1

nb_hopitaux_capacite_limite = 0
if status_occ_h1 == "[ALERTE ELEVEE]":
    nb_hopitaux_capacite_limite += 1
if status_occ_h2 == "[ALERTE ELEVEE]":
    nb_hopitaux_capacite_limite += 1
if status_occ_h3 == "[ALERTE ELEVEE]":
    nb_hopitaux_capacite_limite += 1
if status_occ_h4 == "[ALERTE ELEVEE]":
    nb_hopitaux_capacite_limite += 1
if status_occ_h5 == "[ALERTE ELEVEE]":
    nb_hopitaux_capacite_limite += 1

nb_hopitaux_capacite_optimale = 0
if status_occ_h1 == "[SITUATION OPTIMALE]":
    nb_hopitaux_capacite_optimale += 1
if status_occ_h2 == "[SITUATION OPTIMALE]":
    nb_hopitaux_capacite_optimale += 1
if status_occ_h3 == "[SITUATION OPTIMALE]":
    nb_hopitaux_capacite_optimale += 1
if status_occ_h4 == "[SITUATION OPTIMALE]":
    nb_hopitaux_capacite_optimale += 1
if status_occ_h5 == "[SITUATION OPTIMALE]":
    nb_hopitaux_capacite_optimale += 1

nb_hopitaux_sous_utilisation = 0
if status_occ_h1 == "[SOUS-UTILISATION]":
    nb_hopitaux_sous_utilisation += 1
if status_occ_h2 == "[SOUS-UTILISATION]":
    nb_hopitaux_sous_utilisation += 1
if status_occ_h3 == "[SOUS-UTILISATION]":
    nb_hopitaux_sous_utilisation += 1
if status_occ_h4 == "[SOUS-UTILISATION]":
    nb_hopitaux_sous_utilisation += 1
if status_occ_h5 == "[SOUS-UTILISATION]":
    nb_hopitaux_sous_utilisation += 1
...
print(
    f"  Hôpitaux en état de saturation       : {nb_hopitaux_saturation} / 5 établissement(s)"
)
print(
    f"  Hôpitaux à capacité limite           : {nb_hopitaux_capacite_limite} / 5 établissement(s)"
)
print(
    f"  Hôpitaux à capacité optimale         : {nb_hopitaux_capacite_optimale} / 5 établissement(s)"
)
print(
    f"  Hôpitaux en sous-utilisation active  : {nb_hopitaux_sous_utilisation} / 5 établissement(s)"
)
print("=" * 65)
...
if nb_hopitaux_saturation > 0:
    print(
        f"  !! ALERTE PRIORITAIRE : {nb_hopitaux_saturation} structure(s) saturée(s) — transferts à organiser d’urgence !!"
    )
    print("=" * 65)
else:
    print(
        "  Félicitations : Aucune saturation n’est à déplorer dans les 5 établissements suivis."
    )
...
print(
    f"{'Département cible':<10}\t| {'Population cible':<5}\t| {'Personnes vaccinées':<5}\t| {'Taux de vaccination':<26}\t| {'Statut (Normes OMS)':<5}\t| {'Objectif d’action':<0}"
)
print("-" * 160)
print(
    f"{departement_1:<20}\t| {population_cible_1:<15}\t| {personnes_vaccinees_1:<15}\t| {taux_vaccination_d1:<26}%\t| {status_vac_h1:<5}\t| {action_vac_1:<0}"
)
print("-" * 160)
print(
    f"{departement_2:<20}\t| {population_cible_2:<15}\t| {personnes_vaccinees_2:<15}\t| {taux_vaccination_d2:<26}%\t| {status_vac_h2:<5}\t| {action_vac_2:<0}"
)
print("-" * 160)
print(
    f"{departement_3:<20}\t| {population_cible_3:<15}\t| {personnes_vaccinees_3:<15}\t| {taux_vaccination_d3:<26}%\t| {status_vac_h3:<5}\t| {action_vac_3:<0}"
)
print("-" * 160)
print(
    f"{departement_4:<20}\t| {population_cible_4:<15}\t| {personnes_vaccinees_4:<15}\t| {taux_vaccination_d4:<26}%\t| {status_vac_h4:<5}\t| {action_vac_4:<0}"
)
print("-" * 160)
...
if status_vac_h1 == "[ZONE CRITIQUE]":
    nb_departement_camp_urg += 1
if status_vac_h2 == "[ZONE CRITIQUE]":
    nb_departement_camp_urg += 1
if status_vac_h3 == "[ZONE CRITIQUE]":
    nb_departement_camp_urg += 1
if status_vac_h4 == "[ZONE CRITIQUE]":
    nb_departement_camp_urg += 1

nb_departement_camp_renforce = 0
if status_vac_h1 == "[ZONE A RISQUE]":
    nb_departement_camp_renforce += 1
if status_vac_h2 == "[ZONE A RISQUE]":
    nb_departement_camp_renforce += 1
if status_vac_h3 == "[ZONE A RISQUE]":
    nb_departement_camp_renforce += 1
if status_vac_h4 == "[ZONE A RISQUE]":
    nb_departement_camp_renforce += 1

nb_departement_camp_zone_insuf = 0
if status_vac_h1 == "[ZONE INSUFFISANTE]":
    nb_departement_camp_zone_insuf += 1
if status_vac_h2 == "[ZONE INSUFFISANTE]":
    nb_departement_camp_zone_insuf += 1
if status_vac_h3 == "[ZONE INSUFFISANTE]":
    nb_departement_camp_zone_insuf += 1
if status_vac_h4 == "[ZONE INSUFFISANTE]":
    nb_departement_camp_zone_insuf += 1

nb_departement_camp_zone_conforme = 0
if status_vac_h1 == "[ZONE OMS CONFORME]":
    nb_departement_camp_zone_conforme += 1
if status_vac_h2 == "[ZONE OMS CONFORME]":
    nb_departement_camp_zone_conforme += 1
if status_vac_h3 == "[ZONE OMS CONFORME]":
    nb_departement_camp_zone_conforme += 1
if status_vac_h4 == "[ZONE OMS CONFORME]":
    nb_departement_camp_zone_conforme += 1
...
print(
    f"  Départements requérant une campagne d’urgence obligatoire : {nb_departement_camp_urg} / 4 département(s)"
)
print(
    f"  Départements requérant une campagne renforcée de suivi     : {nb_departement_camp_renforce} / 4 département(s)"
)
print(
    f"  Départements sous le seuil attendu (objectif OMS non atteint) : {nb_departement_camp_zone_insuf} / 4 département(s)"
)
print(
    f"  Départements parfaitement conformes aux normes de l’OMS     : {nb_departement_camp_zone_conforme} / 4 département(s)"
)
print("=" * 65)
...
if nb_departement_camp_zone_insuf > 0:
    print(
        f"  !! ALERTE PRIORITAIRE : {nb_departement_camp_zone_insuf} département(s) nécessite(nt) un renforcement immédiat pour atteindre les seuils recommandés !!"
    )
    print("=" * 65)
else:
    print(
        "  Félicitations : Tous les départements atteignent une couverture vaccinale optimale."
    )
