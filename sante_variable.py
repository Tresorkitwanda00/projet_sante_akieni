...
#DONNEES  DE COUVERTURE VACINALE PAR Departement
...
# REGLE a suivre — STOCK MEDICAMENT :
# Si stock <= seuil_rupture => STATUT : RUPTURE CRITIQUE (alerte immediate PNA)
# Si stock <= seuil_rupture * 1.5 => STATUT : ALERTE STOCK (commande urgente a declencher)
# Si stock <= seuil_rupture * 2 => STATUT : STOCK LIMITE (surveillance renforcee)
# Sinon => STATUT : STOCK NORMAL

# Médicament 1
 
if m1_stock <= m1_seuil_rupture:
    m1_statut, m1_couleur, m1_action = 'RUPTURE CRITIQUE', '[ROUGE]', 'Alerte immediate PNA — commande express sous 24h'
elif m1_stock <= m1_seuil_rupture * 1.5:
    m1_statut, m1_couleur, m1_action = 'ALERTE STOCK', '[ORANGE]', 'Commande urgente a declencher sous 72h'
elif m1_stock <= m1_seuil_rupture * 2.0:
    m1_statut, m1_couleur, m1_action = 'STOCK LIMITE', '[JAUNE]', 'Surveillance renforcee-planifier commande'
else:
...
if m2_stock <= m2_seuil_rupture:
    m2_statut, m2_couleur, m2_action = 'RUPTURE CRITIQUE', '[ROUGE]', 'Alerte immediate PNA — commande express sous 24h'
elif m2_stock <= m2_seuil_rupture * 1.5:
    m2_statut,m2_couleur , m2_action = 'ALERTE STOCK', '[ORANGE]', 'Commande urgente a declencher sous 72h'
elif m2_stock <= m2_seuil_rupture * 2.0:
    m2_statut, m2_couleur, m2_action = 'STOCK LIMITE', '[JAUNE]', 'Surveillance renforcee-planifier commande'
else:
...
if m3_stock <= m3_seuil_rupture:
    m3_statut, m3_couleur, m3_action = 'RUPTURE CRITIQUE', '[ROUGE]', 'Alerte immediate PNA — commande express sous 24h'
elif m3_stock <= m3_seuil_rupture * 1.5:
    m3_statut,m3_couleur , m3_action = 'ALERTE STOCK', '[ORANGE]', 'Commande urgente a declencher sous 72h'
elif m3_stock <= m3_seuil_rupture * 2.0:
    m3_statut, m3_couleur, m3_action = 'STOCK LIMITE', '[JAUNE]', 'Surveillance renforcee-planifier commande'
else:
...
if m4_stock <= m4_seuil_rupture:
    m4_statut, m4_couleur, m4_action = 'RUPTURE CRITIQUE', '[ROUGE]', 'Alerte immediate PNA — commande express sous 24h'
elif m4_stock <= m4_seuil_rupture * 1.5:
    m4_statut,m4_couleur , m4_action = 'ALERTE STOCK', '[ORANGE]', 'Commande urgente a declencher sous 72h'
elif m4_stock <= m4_seuil_rupture * 2.0:
    m4_statut, m4_couleur, m4_action = 'STOCK LIMITE', '[JAUNE]', 'Surveillance renforcee-planifier commande'
else:
    m4_statut, m4_couleur, m4_action = 'STOCK NORMAL', '[VERT]', 'Situation normale — suivi standard'


# Médicament 5 
 
if m5_stock <= m5_seuil_rupture:
    m5_statut, m5_couleur, m5_action = 'RUPTURE CRITIQUE', '[ROUGE]', 'Alerte immediate PNA — commande express sous 24h'
elif m5_stock <= m5_seuil_rupture * 1.5:
    m5_statut,m5_couleur , m5_action = 'ALERTE STOCK', '[ORANGE]', 'Commande urgente a declencher sous 72h'
elif m5_stock <= m5_seuil_rupture * 2.0:
    m5_statut, m5_couleur, m5_action = 'STOCK LIMITE', '[JAUNE]', 'Surveillance renforcee-planifier commande'
else:
...
# 1. CALCULS DES KPIS POUR CHAQUE ETABLISSEMENT
...
# ===============================================================================================
# 2. CLASSIFICATION PRÉALABLES DES STATUTS D'OCCUPATION EN FONCTION DE TAUX D'OCCUPATION
# ================================================================================================

# REGLE a suivre — TAUX D'OCCUPATION DES LITS :
# Si taux > 95% => ALERTE CRITIQUE (saturation — transferts a organiser)
# Si taux > 85% => ALERTE ELEVEE (capacite limite — renforcement prevu)
# Si taux > 70% => SITUATION OPTIMALE
# Sinon => SOUS-UTILISATION (ressources sous-exploitees)
...
# 1. CALCULS DES TAUX DE VACCINATION  POUR CHAQUE DEPARTEMENT
...
# ===========================================================================================
# 2. CLASSIFICATION PRÉALABLES DES STATUTS DE VACCINATION EN FONCTION DE TAUX DE VACCINATION 
# ============================================================================================
# REGLE 4 — COUVERTURE VACCINALE DEPARTEMENT :
# Si taux < 50% => ZONE CRITIQUE (campagne d'urgence obligatoire)
# Si taux < 80% => ZONE A RISQUE (campagne renforcee requise)
...
#==== SECTION I : Rapport d'etat global avec alertes (NEW S3) (compteurs + f-strings)
# S3 (nouveau) : statuts et couleurs determines par les conditions
# TODO 1: Rapport de la pharmacie
print('=' * 163)
print(f"{'':<44}\t{'RAPPORT DE STOCK — PHARMACIE NATIONALE D APPROVISIONNEMENT'}")
print('=' * 163)
# les entetes du tableua de bord de stock medicament
print(f"{'Nom medicament ':<30}\t| {'Stock medicament':<5}\t| {'Seuil rupture':<5}\t| {'Statut du Stock':<0}\t| {'Ind.Couleur':<5}\t| {'Action':<0}")
print('-' * 163)
# Medicament 1 affichage
print(f"{m1_nom:<30}\t| {m1_stock:<10}unites\t| {m1_seuil_rupture:<7}unites\t| {m1_statut:<15}\t| {m1_couleur:<5}\t| {m1_action:<15}")
print('-' * 163)
# Medicament 2 affichage
...
print(f'  Ruptures critiques : {nb_ruptures_critiques} ({nom_med_critique})')
print(f'  Alertes stock : {nb_alertes_stock} ({nom_med_alerte_stock})')
print(f'  Stocks Normaux : {nb_stock_normaux} ({nom_med_stock_normaux})')
print(f'  Stocks Limites : {nb_stock_limites} ({nom_med_stock_limite})')
...
# TODO 2 : Afiichage de rapport niveaux d'occupation  des hopitaux 
print('=' * 142)
print(f"{'':<44}\t{'CLASSIFICTAION DES HOSPITAUX SELON LES NIVEAUX D\'OCCUPATIONS'}")
print('=' * 142)
# les entetes du tableua de bord de stock medicament
print(f"{'Nom de l\'etablissement ':<30}\t| {'Lits ':<5}\t| {'Lits occupés':<5}\t| {'Taux d\'occupation':<0}\t| {'Statut OMS attendu':<5}\t| {'Action':<0}")
print('-' * 142)
print(f"{h1_nom:<30}\t| {h1_nb_lits:<5}\t| {h1_nb_lits_occupes:<10}\t| {taux_occupation_h1:<14}%\t| {status_occ_h1:<5}\t| {action_1:<0}")
print('-' * 142)

print(f"{h2_nom:<30}\t| {h2_nb_lits:<5}\t| {h2_nb_lits_occupes:<10}\t| {taux_occupation_h2:<14}%\t| {status_occ_h2:<5}\t| {action_2:<0}")
print('-' * 142)

print(f"{h3_nom:<30}\t| {h3_nb_lits:<5}\t| {h3_nb_lits_occupes:<10}\t| {taux_occupation_h3:<14}%\t| {status_occ_h3:<5}\t| {action_3:<0}")
print('-' * 142)

print(f"{h4_nom:<30}\t| {h4_nb_lits:<5}\t| {h4_nb_lits_occupes:<10}\t| {taux_occupation_h4:<14}%\t| {status_occ_h4:<5}\t| {action_4:<0}")
print('-' * 142)

print(f"{h5_nom:<30}\t| {h5_nb_lits:<5}\t| {h5_nb_lits_occupes:<10}\t| {taux_occupation_h5:<14}%\t| {status_occ_h5:<5}\t| {action_5:<0}")
print('=' * 142)
#======================== Compteurs de selon le statut OMS attendu ========================================================
...
# --- BILAN FINAL ---
print('BILAN DE NIVEAU D\'OCCUPATION — SELON OMS')
print(f'  Hopitaux en saturation : {nb_hopitaux_saturation} / 5 Hopiatux')
print(f'  Hopitaux a capacite limite : {nb_hopitaux_capacite_limite} / 5 Hopiatux')
print(f'  Hopitaux en capacite optimale : {nb_hopitaux_capacite_optimale} / 5 Hopiatux')
print(f'  Hopitaux en ressources sous-exploitees : {nb_hopitaux_sous_utilisation} / 5 Hopiatux')
print('=' * 65)

# Alerte prioritaire conditionnelle 
if nb_hopitaux_saturation > 0:
    print(f'  !! ALERTE PRIORITAIRE : {nb_hopitaux_saturation} saturation — transferts a organise !!')
    print('=' * 65)
else :
    print('Aucune saturation dans les 5 Hopitaux')
================= fin d'affichage rapport de niveau d'occupation

#TODO : affcihage des donnees sur les vaccins 
print('=' * 160)
print(f"{'':<44}\t{'DONNEES DE COUVERTURE VACCINALE PAR DEPARTEMENT'}")
print('=' * 160)
# les entetes du tableua de bord de stock medicament
print(f"{'Nom du departement ':<10}\t| {'Population Cible ':<5}\t| {'Personnes vaccinées':<5}\t| {'Taux de la vaccination':<0}\t| {'Statut OMS attendu':<5}\t| {'Objectf':<0}")
print('-' * 160)
...
# --- BILAN FINAL ---
print('BILAN DE COUVERTURE VACCINALE PAR DEPARTEMENT — SELON OMS')
print(f' Departement ayant besoin campagne d\'urgence obligatoire : {nb_departement_camp_urg} / 4 Departements')
print(f' Departement ayant besoin campagne renforcee requise : {nb_departement_camp_renforce} / 4 Departements')
print(f' Departement n\'ayant pas atteint l\'objectif de l\'OMS  : {nb_departement_camp_zone_insuf} / 4 Departements')
print(f' Departement conforme : {nb_departement_camp_zone_conforme} / 4 Departements')
print('=' * 65)

# Alerte prioritaire conditionnelle 
if nb_departement_camp_zone_insuf > 0:
    print(f'  !! ALERTE PRIORITAIRE : {nb_departement_camp_zone_insuf} ont besoin de la sufissance en couverture de vaccination !!')
    print('=' * 65)
else :
    print('Aucune saturation dans les 5 Departements')
#================= fin d'affichage rapport de vaccin =====================================================================================
#======================================== fin s3===============================================================================================================
